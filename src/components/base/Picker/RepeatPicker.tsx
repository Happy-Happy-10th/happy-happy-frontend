...
const repeatPickerFrame = clsx('w-full h-27 pt-5 pb-5 pl-4 pr-4', 'yoteyo-m-title-main-sm');
    ...
    <div className={repeatPickerFrame}>
      ...
      <div className="flex flex-row gap-4 mt-[18px]">
        {options.map(option => (
          <div
            key={option}
            className={cn(buttonStyle, value === option && 'bg-yoteyo-main text-white')}
            onClick={() => handleSelected(option)}
          >
            {option === 'day' ? '일' : option === 'week' ? '주' : option === 'month' ? '월' : '년'}
          </div>
        ))}
      </div>
    </div>
