...
const calendarKeys = createQueryKeys('calendar', {
  ...
  userSettings: () => ({ queryKey: ['userSettings'] }),
});

const regionData = createQueryKeys('region', {
  sido: () => ({ queryKey: ['sido'] }),
  sigungu: (sidoCode: string) => ({ queryKey: [sidoCode] }),
});
 ...
 */
export const queryKeys = mergeQueryKeys(calendarKeys, productsKeys, regionData);
